...
typeDefImpl (TypeDef name (SumParts parts)) = [printf "pub enum %s {" $ composite name] ++ fmap (\(n, t) -> printf "    %s(%s)," (composite n) (type_ t)) parts ++ ["}"]
typeDefImpl (TypeDef name (ProdParts parts)) = [printf "pub struct %s {" $ composite name] ++ fmap (\(n, t) -> printf "    %s: %s," (member n) (type_ t)) parts ++ ["}"]
