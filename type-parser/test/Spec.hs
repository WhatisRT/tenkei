...
parsedTestLib = Just $ DefFile ["test","module"] [
...
main = hspec $ do
  describe "Haskell parser" $ do
    ...
    it "parses more complicated haskell files" $
      fmap parseHaskell (readFile "test/testLib.hs") >>= (`shouldBe` parsedTestLib)

  describe "Rust parser" $ do
    it "parses more complicated Rust files" $
      fmap parseRust (readFile "test/testLib.rs") >>= (`shouldBe` parsedTestLib)
